...
from classifiers_twitter.download_tweets import download_tweets
...
def open_classifier(file_name, classifier_folder='classifiers'):
    file_path = file_name if file_name is not None else f"{classifier_folder}/{file_name}.classifier"
    with open(file_path, 'rb') as output:
        return pickle.load(output)


def multi_classifier_predict(classifier_list, texts):
    label_counter = {}
    label_confidences = {}

    translator = Translator()
    for input_string in texts:

        try:
            input_string = translator.from_any_to_pt(input_string)
        except Exception as e:
            continue
        ...
        for classifier in classifier_list:
            ...
            if label not in label_counter or label not in label_confidences:
                label_counter[label] = 0
                label_confidences[label] = 0

            label_counter[label] += 1
            label_confidences[label] += max(classifier.confidence(input_string)[0])

    # for label in label_counter:
    #    label_confidences[label] *= label_counter[label]

    return dict(sorted(list(label_confidences.items()), key=lambda l: l[1], reverse=True)), label_counter
