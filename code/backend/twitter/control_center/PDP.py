...
import log_actions
import classifiers_twitter.utils
import os
...
class PDP:
        ...
        if msg_type == PoliciesTypes.REQUEST_TWEET_LIKE:
            ...
            2- Check neo4j
                ...
                2.3- No one follows tweet_user_id:
                        return PERMIT
            '''
            evaluate_answer = self.analyze_follow_user(msg) > THRESHOLD_FOLLOW
        if evaluate_answer:
    ...
    def get_first_time_list(self):
        ...
        """
        users = [
            "_cdspp", "ppdpsd", "RuiRioPSD", 'Alexandre_Poco', 'anamiguel1981', 'adcacunha', 'malodeabreu',
            'catarinarf', 'cludiaandr1', 'cristovaonorte', 'DuarteMarques', 'francisco__rs', '_jalmeida_',
            'cristasassuncao'
        ]
    ...
    def analyze_follow_user(self, data):
        ...
        @returns: float that will then be compared to the threshold previously defined
        """
        # This was not implemeneted last year (ofc)

        sys.modules['utils'] = classifiers_twitter.utils  # temporario (remover depois)

        heuristic = 0

        policy = self.postgres.search_policies({
            "bot_id": data["bot_id"]
        })

        policy_data = policy['data'][0]  # Um bot pode ter varias policies assigned ???
        ...
        if policy_data:
            ...
            if data['is_protected']:
                if data['profile_score'] >= 1:
                    heuristic += 0.1
                else:
                    heuristic -= 0.3

            if len(description + tweets) > 0:
                classifier_path = 'classifiers_twitter/classifiers_objects'
                classifiers = []

                for file_name in os.listdir(classifier_path):
                    classifiers.append(classifiers_twitter.utils.open_classifier(f"{classifier_path}/{file_name}"))

                confidence, counter = classifiers_twitter.utils.multi_classifier_predict(classifiers,
                                                                                         description + tweets)
                label_name = list(confidence.keys())[0]
                label_confidence = confidence[label_name]

                if policy_data['filter'] == 'Keywords':
                    if label_name in policy_data['params']:
                        heuristic += (label_confidence / counter[label_name])

                elif policy_data['filter'] == 'Target':
                    pass
