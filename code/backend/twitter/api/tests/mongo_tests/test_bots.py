...
@catch_exception
def test_successful_twitter_bot_request(error_catcher, factory, user):
	assert add_bot_neo4j()
	path = reverse('twitter_bot', kwargs={'bot_id': "1"})
	request = factory.get(path)
	response = bots.twitter_bot(request, bot_id="1")
	assert is_response_successful(response)


@catch_exception
def test_unsuccessfully_twitter_bot_request(error_catcher, factory, db):
	path = reverse('twitter_bot', kwargs={'bot_id': "1"})
	request = factory.get(path)
	response = bots.twitter_bot(request, bot_id="1")
