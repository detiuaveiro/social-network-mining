...
@pytest.fixture
def policies_list(db):
	return mixer.cycle(20).blend(Policy, tags=["PSD", "PS"], bots=[1])
...
@catch_exception
def test_successful_bot_policies_request_with_pagination(error_catcher, factory, policy):
	path = reverse('bot_policies', kwargs={'bot_id': 1, 'entries_per_page': 1, 'page': 1})
	request = factory.get(path)
	response = policies.bot_policies(request, bot_id=1, entries_per_page=1, page=1)
	assert is_response_successful_with_pagination(response, 1)


@catch_exception
def test_empty_bot_policies_request(error_catcher, factory, db):
	path = reverse('bot_policies', kwargs={'bot_id': 1})
	request = factory.get(path)
	response = policies.bot_policies(request, bot_id=1)
...
@catch_exception
def test_unsuccessfully_bot_policies_request(error_catcher, factory, policy):
	path = reverse('bot_policies', kwargs={'bot_id': 1, 'entries_per_page': 0, 'page': 1})
	request = factory.get(path)
	response = policies.bot_policies(request, bot_id=1, entries_per_page=0, page=1)
	assert is_response_unsuccessful(response)
