...
class TwitterBot(RabbitMessaging):
    ...
    def __read_timeline(self, user: User, jump_users: bool = False, max_depth: int = 3, current_depth: int = 0):
        ...
        for tweet in tweets:
            ...
            if not jump_users:
                ...
                tweet_user_score = (tweet_user.followers_count + tweet_user.friends_count +
                                    tweet_user.statuses_count) / 3
                bot_user_score = (self.user.followers_count + self.user.friends_count +
                                  self.user.statuses_count) / 3

                follow_user_params = {
                    'id': tweet_user.id,
                    'description': [tweet_user.description],
                    'tweets': [],
                    'is_protected': tweet_user.protected,
                    'profile_score': bot_user_score / tweet_user_score,
                }

                if 'suspended' in user_attributes and tweet_user.suspended:
                    logger.warning(f"Found user with ID={tweet_user.id} but his account was suspended, "
                                   f"so can't read his timeline")
                    return

                elif 'following' in user_attributes and tweet_user.protected and not tweet_user.following:
                ...
                if not tweet_user.protected:
                    tweets_id = [t._json['id'] for t in self.__user_timeline_tweets(
                        tweet_user, count=MAX_NUMBER_TWEETS_FOLLOWING_DECESION)]
                    follow_user_params['tweets'] = [self.__find_tweet_by_id(tweet_id)._json['full_text']
                                                    for tweet_id in tweets_id]

                    self.__read_timeline(tweet_user, jump_users=jump_users,
                                         max_depth=max_depth,
                                         current_depth=current_depth + 1)  # Read tweet_user timeline

                self.__send_query(follow_user_params,
                                  messages_types.BotToServer.QUERY_FOLLOW_USER)  # Request a follow user
            ...
            if 'following' in user_attributes and tweet_user.protected and not tweet_user.following:
                ...
                # self.__send_query(follow_user_params, messages_types.BotToServer.QUERY_FOLLOW_USER)
                continue
    ...
    def __find_tweet_by_id(self, tweet_id: int) -> Union[Status, None]:
        ...
        """
        try:
            return self._twitter_api.get_status(tweet_id, tweet_mode='extended')
        except TweepError as error:
