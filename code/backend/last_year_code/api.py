...
# This is a basic Flask API that connects our multiple DBs

# Question: Why the FUCK é q só alguns dos routes é que tem o parametro method=(...), isso n devia ser especificado em todos?
...
# Route to a page that shows a map of all users' id (Uses the Mongo DB)
...
# Route to a page that shows all stats of all users	(Uses the Postgres DB)
# Note: Is this viable? pq a mim cheira-me q isto ia criar uma pagina lenta q fode
...
# Route to page that shows info on a user with a given id (Uses the Mongo DB)
...
# Routes to a page that shows all tweets of a given user (?) (Uses the Mongo DB)
    ...
    for i in mapa:
        ...
        if i["is_quote_status"]:
            try:
                ...
                pass
        if type(i["in_reply_to_screen_name"]) is str:  # <- wtf is this
            i["in_reply_to_user_id"] = str(i["in_reply_to_user_id"])
            i["in_reply_to_status_id"] = str(i["in_reply_to_status_id"])
    return jsonify(mapa)


# Routes to a page that shows all followers of a given user (Uses the Neo4j DB)
...
# Routes to a page with all users following a given user (Uses the Neo4j DB)
...
# Routes to a page containing all stats of a given user (Uses the Postgres DB)
...
# Used to export all user's info as a JSON, CSV or Dictionary (?)
...
def export_users():
    if request.method == 'GET':
        ...
        if exportype is None:
            if export_fields is None:
                ...
                try:
                    ...
                    return app.response_class(response=json.dumps({"Error": "Wrong field specified"}), status=400,
                                              mimetype='application/json')
        ...
        return app.response_class(response=json.dumps({"Error": "Method not allowed"}), status=405,
                                  mimetype='application/json')
...
# Used to export the entire network as CSV, GraphML, or..string i guess? like wtf it just returns str(fi)
...
def export_network():
    if request.method == "GET":
        ...
        if exportype is None:
            ...
            return app.response_class(response=json.dumps({"Error": "Type not accepted"}), status=400,
                                      mimetype='application/json')
    else:
        return app.response_class(response=json.dumps({"Error": "Method not allowed"}), status=405,
                                  mimetype='application/json')
