...
class postgreSQL_API:
    ...
    def __init__(self, databaseName):
        ...
        try:
            ...
            self.conn = psycopg2.connect(host=settings.POSTGRES_URL, database=databaseName,
                                         user=settings.POSTGRES_USERNAME, password=settings.POSTGRES_PASSWORD)
    ...
    def addUser(self, mapa):
        ...
        try:
            ...
            cur = self.conn.cursor()
            cur.execute("insert into users (timestamp, user_id, followers, following) values (DEFAULT,%s,%s,%s);",
                        (mapa["user_id"], mapa["followers"], mapa["following"]))
            self.conn.commit()
            cur.close()
        except psycopg2.Error as e:
            self.conn.rollback()
    ...
    def getPoliciesByAPI(self, api):
        ...
        try:
            cur = self.conn.cursor()
            cur.execute(
                "select api.name,filter.name,policies.name,policies.params,policies.active,policies.id_policy from policies left outer join filter on filter.id=policies.filter left outer join api on api.id=policies.API_type where api.name=%s;",
                (api,))
            data = cur.fetchall()
            self.conn.commit()
    ...
    def getPoliciesByID(self, id):
        ...
        try:
            cur = self.conn.cursor()
            cur.execute(
                "select api.name,policies.name,params,active,id_policy,filter.name from policies left outer join filter on filter.id=policies.filter left outer join api on api.id=policies.API_type where policies.id_policy=%s;",
                (id,))
            data = cur.fetchall()
            self.conn.commit()
            result = self.postProcessResults(data, ['API_type', 'name', 'params', 'active', 'id_policy', 'filter'])
    ...
    def addLog(self, mapa):
        ...
        try:
            # add log
            cur = self.conn.cursor()
            cur.execute("insert into logs (id_bot,timestamp,action) values (%s,DEFAULT,%s);",
                        (mapa["id_bot"], mapa["action"]))
            self.conn.commit()
            cur.close()
        except psycopg2.Error as e:
            self.conn.rollback()
    ...
    def addPolicy(self, mapa):
        ...
        try:
            ...
            if "active" in mapa.keys():
                cur.execute("insert into policies (API_type,filter,name,params,active) values (%s,%s,%s,%s,%s);",
                            (api, filtro, mapa["name"], mapa["params"], mapa["active"]))
            else:
                cur.execute("insert into policies (API_type,filter,name,params) values (%s,%s,%s,%s);",
                            (api, filtro, mapa["name"], mapa["params"]))
            self.conn.commit()
            cur.close()
        except psycopg2.Error as e:
            self.conn.rollback()
        ...
        try:
            ...
            for i in mapa:
                if i == "API_type":
                    ...
                    cur.execute("update policies set filter=%s where policies.id_policy=%s",
                                (filtro, mapa["id_policy"]))
                elif i == "name":
                    cur.execute("update policies set name=%s where policies.id_policy=%s", (mapa[i], mapa["id_policy"]))
                elif i == "params":
                    cur.execute("update policies set params=%s where policies.id_policy=%s",
                                (mapa[i], mapa["id_policy"]))
                elif i == "active":
                    cur.execute("update policies set active=%s where policies.id_policy=%s",
                                (mapa[i], mapa["id_policy"]))
                else:
                    pass
            self.conn.commit()
    ...
    def getPoliciesParams(self):
        ...
        try:
            cur = self.conn.cursor()
            cur.execute(
                "select params from policies,filter where policies.active=TRUE and policies.filter=filter.id and filter.name='Target'")
            DB_val = cur.fetchall()
            self.conn.commit()
            cur.close()
            return DB_val
